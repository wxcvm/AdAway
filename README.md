...
- **仓库维护者/打包前统一换默认图**：
  ```bash
  pip install Pillow --break-system-packages  # 如果还没装
  python3 scripts/set_default_block_images.py your_image.png
  ```
  支持 1 张图（套用到全部 7 个槽位）或最多 7 张图（一一对应，`img_00.webp`..`img_06.webp`）。这几张图不要求统一尺寸/比例（原图本身就是 512x768、512x512、512x410混着的）。改完需要重新构建才会生效。
