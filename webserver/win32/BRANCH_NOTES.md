...
- 发布后远程验证（v1.44，审计第68条）：CI 以前在覆盖上传 fixed-tag 清单后就结束，
  “发布了但固定标签还指向上一版”只有用户点检查更新时才会发现。现在清单步骤
  结束后会**回读**远端清单（按客户端同样的方式带时间戳绕过 CDN 缓存），
  断言其 tag 等于本次的 TAG；最多重试 8 次（每次间隔 20 秒，给 CDN 更新留时间），
  仍不一致就让构建失败并给出明确错误。
  审计第67条（CI 不要吞掉失败）这块按“有害才去”处理：产物上传与发布链路的
