...
日志与配置：
  ...
  * webserver.ini      保存 http/https 端口、bind_all、theme；
                       ...
                       连不上 GitHub 时还可以写 mirror=（GitHub 加速站），
                       例如 mirror=https://ghfast.top/ —— 清单和更新包都会
                       改成从加速站取，下载后仍然强制校验发布方的
                       SHA-256，加速站无法替换成别的程序；
                       每次检查的线路、HTTP 状态码与耗时都会写进
                       webserver.log，失败时先看那几行就能知道卡在哪；
    证书信任：设置页“信任 CA”会同时写入“当前用户根”和（有管理员权限时的）
              “本机根”，装完会回读校验，状态栏直接显示“已信任（用户根 +
              本机根）”这类结果；只想覆盖当前账户时用普通权限运行即可；
    最近请求：保留最新 400 条，卡片里用鼠标滚轮翻阅，右上角显示总条数。
    拦截方式：设置页每个类型一个按钮，点击在 占位响应 → 空响应 → 204 →
              不拦截 之间循环，旁边的“恢复推荐默认”一键回到推荐值
              （看得见占位图的用占位；脚本/样式/字体用空响应；
              API/遥测/配置/WebSocket 用 204）。改完立即生效，不用重启，
              查询日志的“方式”列会显示实际用的哪一种。
    更新安全：便携版更新不再直接覆盖目录，而是“旧目录改名备份 → 新版本就位 →
              回填你的数据 → 启动观察 5 秒 → 失败自动回滚旧版本”；
    /control 控制接口需要 resources/control_token.txt 里的随机令牌
              （仪表盘自动带上，用 curl 时加 ?token=<令牌> 或
              -H "X-ADBlock-Token: <令牌>"）。
