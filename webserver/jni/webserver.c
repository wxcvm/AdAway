...
static void block_set_load_subscriptions(const char *resource_dir) {
    ...
    FILE *fp = fopen(path, "r");
    if (!fp) return;
    /* Hosts form (0.0.0.0 domain) and AdGuard form (|| domain ^) both reduce to
       one bare domain per line, which is the only shape we write here; the
       parser therefore stays deliberately small. Written without escape
       characters on purpose (the previous version broke the build). */
    char line[512];
    size_t n = 0;
    while (fgets(line, sizeof(line), fp)) {
        char *s = line;
        while (*s == 0x20 || *s == 0x09) s++;
        if (*s == 0 || *s == 0x23 || *s == 0x21 || *s == 0x0d || *s == 0x0a) continue;
        if (s[0] == 0x7c && s[1] == 0x7c) {
            s += 2;
        } else {
            char *sp = s;
            while (*sp && *sp != 0x20 && *sp != 0x09) sp++;
            if (*sp) { while (*sp == 0x20 || *sp == 0x09) sp++; s = sp; }
        }
        char *end = s;
        while (*end && *end != 0x20 && *end != 0x09 && *end != 0x0d && *end != 0x0a &&
               *end != 0x5e && *end != 0x2f && *end != 0x24 && *end != 0x7c) end++;
        size_t len = (size_t) (end - s);
        if (len == 0 || len > 253 || s[0] == 0x2e) continue;
        if (memchr(s, 0x2e, len) == NULL) continue;   /* single labels are not hosts */
